...
const buildArrayOfPigLatinWord = (wordArray) => {
	return wordArray.map(word => {
		...
		const reorderedWordAsString = reorderedWordAsArray.reduce((a, b) => { // reduces reordered word array to a single string
...
const buildStringFromArrayOfStrings = (stringArray) => {
	return stringArray.reduce((a, b) => {
		return a + ' ' + b // Concatenates two strings, seperated by a space
	})
}
