...
let parseStringToNumber = (string) => {
...
let determineClosestFutureLeapYear = (year) => {
	if(!isLeapYear(year)) {
...
let determineNextLeapYearGivenCurrentLeapYear = (leapYear) => {
	let potentialNextLeapYear = leapYear + 4 // Declares the potential next leap year by incrementing the passed year by four
...
let pushLeapYearToLeapYearArray = (leapYear, leapYearArray) => {
...
let buildLeapYearArray = (currentLeapYear, leapYearArray = []) => {
...
let convertArrayOfIntegersToArrayOfStrings = (arrayOfIntegers) => {
...
let buildStringFromArrayOfStrings = (stringArray) => {
...
let runLeapYearProgram = (userInput) => {
